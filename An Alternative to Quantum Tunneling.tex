...
Alternatively, every entry of the matrices $A\in\mathbb{R}^{n\times m}$ and $B \in\mathbb{R}^{n\times m}$ can be embedded into the upper half-plane $\mathbb{H}^2$. That is, the entries of $A$ and $B$ become complex number with positive imaginary part: $a_{ki}\in\mathbb{H}^2$ and $b_{il}\in\mathbb{H}^2$.
...
$$
\left( M(A) \odot_{\mathbb{H}^2} M(B)\right)_{kl} := \sum\limits_{i=1}^m d_{\mathbb{H}^2} \left(M \left( a_{ki} \right) ,M\left( b_{il}\right) \right) =  \sum\limits_{i=1}^m d_{\mathbb{H}^2} \left( a_{ki}  , b_{il} \right)  =\left( A\odot_{\mathbb{H}^2} B\right)_{kl}.
$$
\end{definition}


\section{Example}
All layers of a given neural network model can be implemented the above hyperbolic-orbit algorithm. The following is an example when the hyperbolic-orbit algorithm is applied to self-attention mechanism, i.e. transformer-based models.
